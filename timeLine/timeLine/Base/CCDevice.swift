...
open class CCDevice {
    //屏幕信息
    static public let w : CGFloat = UIScreen.main.bounds.size.width
    static public let h : CGFloat = UIScreen.main.bounds.size.height
    static public let pixel : CGFloat = 1.0 / UIScreen.main.scale //一个像素的宽度
    static public let scale : CGFloat = UIScreen.main.scale
    static public let zoom : (w:CGFloat, h:CGFloat) = ((CCDevice.w >= 375.0) ? 1.0 : (CCDevice.w / 375.0),
                                                       (CCDevice.h >= 667.0) ? 1.0 : (CCDevice.h / 667.0)) //缩放系数
    ...
    //根据尺寸判断是不是某个设备的尺寸
    static public let iPhone4 : Bool = (CCDevice.w == 320 && CCDevice.h == 480) //640*960  3.5c
    static public let iPhone5 : Bool = (CCDevice.w == 320 && CCDevice.h == 568) //640*1136 4.0C
    static public let iPhone8 : Bool = (CCDevice.w == 375 && CCDevice.h == 667) //750*1334 4.7C
    static public let iPhone8Plus : Bool = (CCDevice.w == 414 && CCDevice.h == 736) //1242*2208 5.5C
    
    static public let iPhoneXs : Bool = (CCDevice.h == 812) //2436*1125 //5.8C
    static public let iPhoneXsMax : Bool = (CCDevice.w == 414 && CCDevice.h == 896 && CCDevice.scale == 3) //2688*1242 6.5C
    static public let iPhoneXr : Bool = (CCDevice.w == 414 && CCDevice.h == 896 && CCDevice.scale == 2)//1792*828 6.1C
    ...
    static public var isXDisplay: Bool {
        return CCDevice.insets.bottom > 0.0
    }
    
    
    //根据isXDisplay处理UI常量信息
    static public var topOffset : CGFloat {
        return CCDevice.insets.top + 44.0
    }
    
    static public var bottomOffset : CGFloat {
        return CCDevice.insets.bottom
    }
    
    static public var naviOffset : CGFloat {
        return CCDevice.insets.top
    }
